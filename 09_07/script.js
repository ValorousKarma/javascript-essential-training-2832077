...
const changeLength = function (e, strap) {
	// stops page from reloading upon event
	e.preventDefault();
	// get new straplength value
	let newValue = strap.querySelector("form input").value;
	// set displayed length value
	strap.querySelector("span").innerText = `${newValue} inches`;
	// reset input box
	strap.querySelector("form input").value = "";
};

// iterate through each backpack
backpackList.forEach((backpack) => {
	const straps = backpack.querySelectorAll(".backpack__strap");

	// iterate through each strap
	straps.forEach((strap) => {
		let side = strap.getAttribute("data-side");

		const lengthAdjustor = document.createElement("form");

		//create input form
		const input = document.createElement("input");
		input.setAttribute("type", "number");
		input.setAttribute("name", `${side}Length`);
		input.setAttribute("placeholder", `New ${side} length`);

		// create actual button
		const submit = document.createElement("button");
		submit.innerText = "Update";

		// add input box and submit button to form element
		lengthAdjustor.append(input, submit);

		lengthAdjustor.addEventListener("submit", (e) => {
			// update the displayed strap length
			changeLength(e, strap);
		});

		// inject the html
		strap.append(lengthAdjustor);
	});
});
